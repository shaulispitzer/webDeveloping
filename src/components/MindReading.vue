...
let questions = [
  {
    question:
      "Divide the number by 3, and type the remainder below. If the remainder is 0, type 0 instead.",
    max: 2,
    multiplyBy: 70,
    position: "first",
  },
  {
    question: "Divide the original number by 5, and type the remainder here.",
    max: 4,
    multiplyBy: 21,
    position: "second",
  },
  {
    question: "Divide the original number by 7, and write down the remainder.",
    max: 6,
    multiplyBy: 15,
    position: "third",
  },
...
<template>
  <div
    class="bg-blue h-80 text-center p-4 border-2 border-black rounded-lg mx-4 my-3 relative"
  >
    ...
    <Transition
      ><div v-if="asking">
        <Transition name="question">
          <p
            v-if="qDisplay"
            class="transition-all md:text-2xl text-base sm:text-xl xsm:text-lg my-3"
          >
            {{ displayQuestion }}
          </p>
        </Transition>
        <div class="outer absolute bottom-15 w-full">
          <div class="inputAndButton relative inline-block">
            <span
              v-if="!correctAnswer"
              class="text-red-700 text-sm md:text-base sm:text-sm absolute md:w-max sm:top-[5px] top-12 sm:right-[105%] sm:w-48"
              >The Number May Only Be Between 0 and {{ maxAnswer }}</span
            >
            <input
              type="text"
              v-model="inputValue"
              v-focus
              maxlength="1"
              class="mr-2 p-2 rounded-md w-10 border border-gray-600"
            />
            <button
              :class="{
                next: correctAnswer,
                '!text-lg': questionNumber === 'third',
              }"
              class="px-2 border-solid border-2 border-purple-700 disabled:border-purple-400 py-1 text-xl my-auto disabled:bg-slate-300 disabled:text-slate-400 disabled:opacity-70 transition-all"
              :disabled="!correctAnswer"
            >
              {{ questionNumber === "third" ? "Show Answer" : "next" }}
            </button>
          </div>
        </div>
      </div></Transition
    ...
    <div class="mt-5" v-if="!asking">
      ...
      <button
        @click="startTrick"
        class="transition-all hover:-translate-y-0.5 bg-white hover:bg-gray-100 text-gray-800 font-semibold mt-1 py-1 px-2 md:px-4 border border-gray-400 rounded shadow"
      >
        start {{ showAnswer ? "again" : "" }}
      </button>
    </div>
  </div>
</template>
